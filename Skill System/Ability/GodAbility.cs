...
[CreateAssetMenu(menuName = "Ability/God")]
public class GodAbility : ScriptableObject
{
    public enum Type
    {
        StunEnemy,
    ...
    public void UseAbility()
    {
        if (battle.player.negativeEffects[2] != null)
        {
            ...
            {
                if (!isCooldown && GameManager.blood >= bloodCost && battle.IsBattleOn())
                {
                    GameObject.Find("Log").GetComponent<BattleLogger>().OnUseAbility(abilityUseReplics);
                    GameManager.blood -= bloodCost;
                    ...
                    if (abilityType == Type.Transformation)
                    {
                        float prevPP = battle.player.physicPower;
                        float prevMP = battle.player.magicPower;
                        float prevHP = battle.player.maxHp;

                        battle.player.physicPower = battle.player.physicPower * value;
                        battle.player.magicPower = battle.player.magicPower * value;
                        battle.player.maxHp = battle.player.maxHp * value;
                        Task.Delay((int)(activeTime * 1000)).ContinueWith(t => ResetTransformation(prevPP, prevMP, prevHP));
                        OnCooldown();
                    }
                    if (abilityType == Type.GetBlood)
                    {
                        GameManager.blood += value;
                        GameManager.UpdateText();
                        OnCooldown();
                    }
                    if (abilityType == Type.MagicStrikeProcent)
                    {
                        battle.DealMagicDmgProcent(battle.player, battle.enemy, value);
                        OnCooldown();
                    }
                }
            }
    ...
    public void ResetTransformation(float prevPP, float prevMP, float prevHP)
    {
        battle.player.physicPower = prevPP;
        battle.player.magicPower = prevMP;
        battle.player.maxHp = prevHP;
    }
}
