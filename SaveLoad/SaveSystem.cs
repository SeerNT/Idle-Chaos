...
public class SaveSystem
{
    ...
    public static void SaveSkin(int currentSkinNum, int lastId)
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/skins.save";
        FileStream stream = new FileStream(path, FileMode.Create);

        SkinData data = new SkinData(currentSkinNum, lastId);
    ...
    public static void SaveConversion(int cost, bool unlocked, string name)
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/conv" + name + ".save";
        FileStream stream = new FileStream(path, FileMode.Create);

        ConversionData data = new ConversionData(unlocked, cost == 1);

        formatter.Serialize(stream, data);
        stream.Close();
    }

    public static ConversionData LoadConversion(string name)
    {
        string path = Application.persistentDataPath + "/conv" + name + ".save";
        if (File.Exists(path))
        {
            ...
            ConversionData data = formatter.Deserialize(stream) as ConversionData;
            stream.Close();
    ...
    public static void SaveTutorial(int dialogue)
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/tutorial.save";
        FileStream stream = new FileStream(path, FileMode.Create);

        TutorialData data = new TutorialData(dialogue);
    ...
    public static void SaveCutscenes(bool[] shown, bool isEnding)
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/ending.save";
        FileStream stream = new FileStream(path, FileMode.Create);
    ...
    public static CutsceneData LoadEndings()
    {
        string path = Application.persistentDataPath + "/ending.save";
        if (File.Exists(path))
        {
    ...
    public static void SaveSeaBattle(double xpDrop, double remainingTime, double timeSpent, int enemyNum)
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/seaBattle.save";
        FileStream stream = new FileStream(path, FileMode.Create);

        SeaBattleData data = new SeaBattleData(xpDrop, remainingTime, timeSpent, enemyNum);

        formatter.Serialize(stream, data);
        stream.Close();
    }

    public static SeaBattleData LoadSeaBattle()
    {
        string path = Application.persistentDataPath + "/seaBattle.save";
        if (File.Exists(path))
        {
            ...
            SeaBattleData data = formatter.Deserialize(stream) as SeaBattleData;
            stream.Close();
    ...
    public static void ResetSeaBattle()
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/seaBattle.save";
        FileStream stream = new FileStream(path, FileMode.Create);

        SeaBattleData data = new SeaBattleData(100, 3600, 0, 1);

        formatter.Serialize(stream, data);
        stream.Close();
    }

    public static void SaveSeaFishing(int itemDropSetting, int rarityDropSetting, int autoLvlSetting, InventorySlot slot)
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/seaFishing.save";
        FileStream stream = new FileStream(path, FileMode.Create);
        SeaFishingData data = null;
        if (slot == null)
        {
            data = new SeaFishingData(itemDropSetting, rarityDropSetting, autoLvlSetting);
        }
        else
        {
            data = new SeaFishingData(itemDropSetting, rarityDropSetting, autoLvlSetting, int.Parse(slot.item.id), slot.lvl);
        }
    ...
    public static SeaFishingData LoadSeaFishing()
    {
        string path = Application.persistentDataPath + "/seaFishing.save";
        if (File.Exists(path))
        {
            ...
            SeaFishingData data = formatter.Deserialize(stream) as SeaFishingData;
            stream.Close();
    ...
    public static void ResetSeaFishing()
    {
        string path = Application.persistentDataPath + "/seaFishing.save";
    ...
    public static void SaveSeaRecycle(int common, int uncommon, int rare, int epic, int legendary, int mythic)
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/seaRecycle.save";
        FileStream stream = new FileStream(path, FileMode.Create);

        SeaRecycleData data = new SeaRecycleData(common, uncommon, rare, epic, legendary, mythic);

        formatter.Serialize(stream, data);
        stream.Close();
    }

    public static SeaRecycleData LoadSeaRecycle()
    {
        string path = Application.persistentDataPath + "/seaRecycle.save";
        if (File.Exists(path))
        {
            ...
            SeaRecycleData data = formatter.Deserialize(stream) as SeaRecycleData;
            stream.Close();
    ...
    public static void ResetSeaRecycle()
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/seaRecycle.save";
        FileStream stream = new FileStream(path, FileMode.Create);

        SeaRecycleData data = new SeaRecycleData(0, 0, 0, 0, 0, 0);
    ...
    public static void ResetWindow()
    {
        ...
        bool[] isShown = new bool[10] {false, false, false, false, false, false, false, true, false, false };
    ...
    public static void SaveHerb(Herb herb, double amount, string name)
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/" + "herb" + name + ".save";
    ...
    public static void SaveSkill(GameAbility skillAbility, string name)
    {
        ...
        {
            data = new SkillData(skillAbility.id, skillAbility.cooldown, battle.startCooldown, skillAbility.isCooldown);
        }
        else
        {
            data = new SkillData(-10, 0, 0, false);
        }
    ...
    public static void SaveItem(GameItem item, string name, int lvl, bool isCollection)
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = "";
        ...
        if (item != null)
        {
            ...
            if (isCollection)
            {
                path = Application.persistentDataPath + "/" + "item_col" + item.id + ".save";
            }
            else
            {
                path = Application.persistentDataPath + "/" + "item_" + name + ".save";
            }
        }
        else
        {
    ...
    public static ItemData LoadItem(string name, bool isCollection)
    {
        string path = Application.persistentDataPath + "/" + "item_col" + name + ".save";
        if (File.Exists(path))
        {
            BinaryFormatter formatter = new BinaryFormatter();
            FileStream stream = new FileStream(path, FileMode.Open);
    ...
    public static void ResetGrowing(Growing obj, string name)
    {
        ...
        growReset.isAuto = false;
        growReset.automationOn = false;
    ...
    public static void ResetSearching(Searching obj, string name)
    {
        ...
        searchReset.isAuto = false;
        searchReset.automationOn = false;
    ...
    public static void SaveOffline(long total)
    {
        BinaryFormatter formatter = new BinaryFormatter();
        string path = Application.persistentDataPath + "/" + "offline.save";
        FileStream stream = new FileStream(path, FileMode.Create);
