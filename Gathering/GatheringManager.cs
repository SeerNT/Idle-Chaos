...
public class GatheringManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            herbStatsObject.transform.GetChild(2).GetChild(0).GetComponent<Text>().text = "Click on herb ©";
        }
    }
    ...
    public void GiveProduct(Growing growingObj, double amount)
    {
        string name = growingObj.name;
        name = name.Replace("Grow", "");

        AddHerbToSlotWithAmount(GetHerb(growingObj), int.Parse(GetHerb(growingObj).id), amount);
    }
    ...
    public bool AddHerbToSlotWithAmount(Herb herb, int slotId, double amount)
    {
        bool isSuccess = false;
        if (slots[slotId - 1] == null)
        {
        ...
        Transform slot = transform.Find("Herbs").Find("Inv" + slotId);
        slot.GetComponent<HerbSlot>().herb = herb;
        slot.GetComponent<HerbSlot>().amount += (int)amount;
